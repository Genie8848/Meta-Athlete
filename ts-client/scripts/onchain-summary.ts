...
(async () => {
  ...
  for (const entry of athletes) {
    if (entry[1].isSome) {
      ...
      console.log(`hahahahahahahah for ${athlete.athleteKind}`);
      const kind = athlete.athleteKind.toUtf8();
      const sports = athlete.athleteSports.toUtf8();
      const birthdate = athlete.athleteBirthdate.toUtf8();
      const birthplace = athlete.athleteBirthplace.toUtf8();
      const height = Number(athlete.height.millimeters.toBigInt()) / 10;
      const weight = Number(athlete.weight.grams.toBigInt()) / 1000;
      console.log(`Registered athlete ${name}: kind=${kind} sports=${sports} birthdate=${birthdate} birthplace=${birthplace} height=${height}cm weight=${weight}kg`)
