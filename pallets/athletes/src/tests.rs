...
use crate::{
  mock::{
    alice, bob, test_ext, Athletes, Balances, Event, Extrinsic, Origin, Runtime, System, TestAuth,
  },
...
use meta_athlete_primitives::{Athlete, AthleteApplication, AthleteCardClass, Height, Weight};
...
fn mint_cards_generates_class_and_cards() {
  test_ext().execute_with(|| {
    ...
    Athletes::submit_athlete_application(Origin::signed(alice()), athlete_application(bob()))
      .unwrap();

    let athlete_id = System::events()
      .iter()
      .filter_map(|record| {
        let event = &record.event;
        if let Event::Athletes(crate::Event::AthleteApplicationSubmitted { id, .. }) = event {
          Some(*id)
        } else {
          None
        }
    ...
    Athletes::approve_application(Origin::root(), athlete_id).unwrap();
    Athletes::mint_cards(Origin::signed(alice()), athlete_id).unwrap();

    dbg!(System::events());

    // Athletes::mint_cards(Origin::signed(alice()), athlete()).unwrap();
    // assert_eq!(Athletes::class_attributes(&0).unwrap(), athlete());
    // assert_eq!(
    //   Athletes::attributes(&0, &0).unwrap(),
    //   AthleteCardAttributes {
    //     price: None,
    //     total_shares: 0,
    //     ty: AthleteCardClass::Platinum,
    //     views: 0,
    //     votes: 0,
    //   }
    // );
    // assert_eq!(
    //   Athletes::attributes(&0, &100).unwrap(),
    //   AthleteCardAttributes {
    //     price: None,
    //     total_shares: 0,
    //     ty: AthleteCardClass::Gold,
    //     views: 0,
    //     votes: 0,
    //   }
    // );
    // assert_eq!(
    //   Athletes::attributes(&0, &140).unwrap(),
    //   AthleteCardAttributes {
    //     price: None,
    //     total_shares: 0,
    //     ty: AthleteCardClass::Platinum,
...
//     t.execute_with(|| {
//       // when
//       Athletes::fetch_pair_prices_and_submit_tx().unwrap();

//       // then
//       let raw_tx = pool_state.write().transactions.pop().unwrap();
//       let tx = Extrinsic::decode(&mut &*raw_tx).unwrap();
//       assert_eq!(tx.signature, None);
//       if let Call::submit_athlete_info { class_id, instance_id, views, votes, signature } = tx.call
//       {
...
//     Athletes::mint_cards(Origin::signed(alice()), athlete()).unwrap();
//     assert_noop!(
//       Athletes::set_card_price(Origin::signed(bob()), 0, 0, Some(10)),
...
//   test_ext().execute_with(|| {
//     Athletes::mint_cards(Origin::signed(alice()), athlete()).unwrap();
//     assert_eq!(Athletes::attributes(&0, &0).unwrap().price, None);
//     Athletes::set_card_price(Origin::signed(alice()), 0, 0, Some(10)).unwrap();
//     assert_eq!(Athletes::attributes(&0, &0).unwrap().price, Some(10));
//     Athletes::set_card_price(Origin::signed(alice()), 0, 0, None).unwrap();
//     assert_eq!(Athletes::attributes(&0, &0).unwrap().price, None);
//   });
// }
...
//     Athletes::mint_cards(Origin::signed(alice()), athlete()).unwrap();
//     assert_noop!(
//       Athletes::transfer_card(Origin::signed(bob()), 0, 0),
...
//     assert_eq!(Athletes::owner(&0, &0).unwrap(), alice());
//     Athletes::transfer_card(Origin::signed(bob()), 0, 0).unwrap();
//     assert_eq!(990, Balances::free_balance(bob()));
//     assert_eq!(previous_alice_balance + 10, Balances::free_balance(alice()));
//     assert_eq!(Athletes::owner(&0, &0).unwrap(), bob());
//   });
// }
...
fn athlete_application<T>(account: T) -> AthleteApplication<T> {
  AthleteApplication {
    name: "John Doe".into(),
    applicant_account: account,
    athleteKind: 0,
    athleteSports: 0,
    athleteBirthdate: "19990909".into(),
    athleteBirthplace: "New York".into(),
    weight: Weight::from_grams(85_000),
    height: Height::from_millimeters(195_000),
    photo: None,
  }
}
